...
func init() {
	...
	containerCmd.Flags().StringVarP(&containerArgs.inputPolicy, "input-policy", "i", "", "Path for the Input policyin Rego, input-policy can be passed from either Local or from remote URL")
	containerCmd.MarkFlagRequired("input-policy")
	containerCmd.Flags().StringVarP(&containerArgs.outputPolicy, "output-policy", "o", "", "Path for Out policy in Rego, Output-policy can be passed from either Local or from remote URL")
...
var containerCmd = &cobra.Command{
	...
	Long:  `A user can pass in a JSON file to genval, the passed input will be first evaluated based on input policies, once passed a Dockerfile will begenerated and the generated Dockerfile will be further validated using the output policies `,
...
func runContainerCmd(cmd *cobra.Command, args []string) error {
	...
	// if containerArgs.reqinput == "" || containerArgs.output == "" || containerArgs.inputPolicy == "" || containerArgs.outputPolicy == "" {
	// 	return errors.New(("[USAGE]: ./genval container --reqinput=input.json --output=output.Dockerfile --inputpolicy=<path/to/input.rego policy> --outputpolicy <path/tp/output.rego file>"))
	// }
